...
export default function PortfolioModal({ entity, onClose }: Props) {
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 15 }}>
            <div style={{
              width: 54, height: 54, borderRadius: 9, background: 'var(--bg)',
              border: '1px solid var(--line-2)', display: 'flex', alignItems: 'center',
              justifyContent: 'center', fontFamily: 'var(--mono)', fontWeight: 600, fontSize: 19,
            ...
            <div>
              ...
              <span style={{
                ...
                color: 'var(--signal)', border: '1px solid var(--line-2)',
        ...
        <div style={{ display: 'flex', gap: 34, marginBottom: 22, flexWrap: 'wrap' }}>
          {[
            { k: '운용 규모', v: entity.aum },
            { k: '보유 종목', v: String(entity.positions) },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 26 }} className="pf-body">
          ...
          <div>
              ...
              {entity.holdings.map((h, i) => (
                ...
                >
                  <div style={{
                    ...
                    fontSize: 11, flexShrink: 0, color: DONUT_PALETTE[i % DONUT_PALETTE.length],
                  }}>{h.t.slice(0, 2)}</div>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ height: 3, background: 'var(--bg)', borderRadius: 2, marginTop: 5, overflow: 'hidden' }}>
                      <span style={{ display: 'block', height: '100%', background: 'var(--white)', width: `${Math.min(h.pct, 100)}%` }} />
                    </div>
                  </div>
                  ...
                  <div style={{ textAlign: 'right', fontFamily: 'var(--mono)', flexShrink: 0 }}>
                    ...
                    {h.chg && (
                      <div style={{
                        ...
                        color: h.chg.includes('+') ? 'var(--white)' : h.chg === '━' ? 'var(--gray-d)' : 'var(--signal)',
                      }}>{h.chg}</div>
                    )}
                  </div>
                </div>
            ...
            <div style={{
              ...
              {entity.kind === 'FUND'
                ? '◆ 13F는 분기별 롱 포지션 스냅샷 — 공매도·옵션·현금·해외주식은 미포함. 최대 45일 시차.'
                : '◆ 개인은 13F 의무 없음 — Form 4(내부자 거래)·13D/G(5%+ 대주주 공시) 기반. 비상장 지분은 추정치이며 정확도가 낮습니다.'}
            </div>
          </div>
        </div>
      </div>
